...
const wfeProcess = {
  ...
  addLink(fromService, toService) {
    if (this.flow.has(fromService.serviceName)) {
      throw Error('Next item for service: ' + fromService.serviceName + ' already defined.');
    } else {
      this.flow.set(fromService.serviceName, {
  ...
  setEndService(service) {
    this.end.add(service.serviceName);
  },
...
module.exports = {
  ...
  createProcess: function (processName) {
    return Object.create(wfeProcess, {
      ...
      services: property(new Map()),
      flow: property(new Map()),
      context: property([]),
      start: property(null, true),
      end: property(new Set())
    });
  },
